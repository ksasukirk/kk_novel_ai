...
| 项 | 值 |
|---|---|
| 当前版本 | `0.2.46`（`package.json` / `src-tauri/Cargo.toml` / `src-tauri/tauri.conf.json` 同步） |
...
| R42 | v0.2.46 写作页按段 / 按章翻译正文（导入不翻全文） | 完成 | [`writing/translate.rs`](src-tauri/src/writing/translate.rs)、[`body_translate.md`](src-tauri/prompts/body_translate.md)、[`ChapterBlockEditor.vue`](src/components/ChapterBlockEditor.vue)、[`EditorView.vue`](src/views/EditorView.vue)、[`draftAccept.js`](src/services/draftAccept.js)、[`api.rs`](src-tauri/src/api.rs)、[`cli.rs`](src-tauri/src/cli.rs)、[`package.json`](package.json)、[`src-tauri/Cargo.toml`](src-tauri/Cargo.toml)、[`src-tauri/tauri.conf.json`](src-tauri/tauri.conf.json)、本文 |

---

## 1. 本版（0.2.46）做了什么

相对 `0.2.45`，导入 TXT 仍只可选翻书名和章标题，**正文保持原文**；打开写作页后，每段粘条可选中 / 英 / 日并点「翻译本段」，工具栏「翻译本章」会按段依次翻写（多段先确认）。长文由后端自动切块，已是目标语则跳过不调模型，译文不由后端写盘、由前端写回并保存。CLI / RPC 提供同一套 `chapter_translate`。npm / Cargo / Tauri 版本对齐到 `0.2.46`。

- **按段翻译**：非插图块粘条增加目标语下拉与「翻译本段」；调用分析模型，成功后写回该段并可撤销。见 [`ChapterBlockEditor.vue`](src/components/ChapterBlockEditor.vue)、[`draftAccept.js`](src/services/draftAccept.js) `translateBlock`。
- **翻译本章**：写作页工具栏选目标语后点「翻译本章」；多段先确认，再按段排队，进度与跳过数会提示。见 [`EditorView.vue`](src/views/EditorView.vue) `onTranslateChapter`、[`draftAccept.js`](src/services/draftAccept.js) `translateOpenChapter`。
- **后端切块与跳过**：`chapter_translate` 按约 2800～3600 字切块；已像目标语则 `skipped`；空译报错。见 [`writing/translate.rs`](src-tauri/src/writing/translate.rs)、三语 [`body_translate.md`](src-tauri/prompts/body_translate.md)、[`prompt_i18n.rs`](src-tauri/src/prompt_i18n.rs) `body_translate_prompt`、[`api.rs`](src-tauri/src/api.rs) / [`commands.rs`](src-tauri/src/commands.rs) / [`cli.rs`](src-tauri/src/cli.rs)。
- **导入说明更新**：作品页导入提示改为「正文请到写作页再翻」。见三语 [`src/i18n/locales/zh-CN.json`](src/i18n/locales/zh-CN.json)、[`en.json`](src/i18n/locales/en.json)、[`ja.json`](src/i18n/locales/ja.json)，[`docs/lmstudio.md`](docs/lmstudio.md)。
- **版本对齐**：[`package.json`](package.json)、[`src-tauri/Cargo.toml`](src-tauri/Cargo.toml)（及 [`Cargo.lock`](src-tauri/Cargo.lock)）、[`src-tauri/tauri.conf.json`](src-tauri/tauri.conf.json) 均为 `0.2.46`。里程碑见 [`docs/todo.md`](docs/todo.md) M72。

**上一版（0.2.45）及更早已交付、本版继续可用的体验**：

- **作品页导入写作工程**：确认书名后只选一次 TXT / Markdown，在 `novels` 下建 `kind=novel` 工程并写入章节；标题翻译默认关，勾选后只翻书名和章标题。见 [`ProjectHome.vue`](src/views/ProjectHome.vue)、[`import/mod.rs`](src-tauri/src/import/mod.rs)、[`title_translate.rs`](src-tauri/src/import/title_translate.rs)。
- **情节卡中英对照**：抽取同轮写 `title_en` / `content_en`；界面英文显示英文，缺则回退中文；续写注入跟写作语言。见 [`tropeI18n.js`](src/utils/tropeI18n.js)、[`TropeLibraryView.vue`](src/views/TropeLibraryView.vue)、[`trope_merge.rs`](src-tauri/src/project/trope_merge.rs)。
- **无标题按段落切章**、空作品 / 空设置能打开、导入 TXT 只选一次文件、作品卡高度完整露出。见 [`import/mod.rs`](src-tauri/src/import/mod.rs)、[`project/mod.rs`](src-tauri/src/project/mod.rs)、[`error.rs`](src-tauri/src/error.rs)、[`KnowledgeHome.vue`](src/views/KnowledgeHome.vue)。
- **中日文统一称「喜好」**、作品页智能搜索、一键重新总结 / 情节卡 AI 优化 / 删卡始终确认、写作页筛选多选情节卡、点书立刻进写作。见 [`ProjectHome.vue`](src/views/ProjectHome.vue)、[`TropePickPanel.vue`](src/components/TropePickPanel.vue)、[`EditorView.vue`](src/views/EditorView.vue)。
- **卡片网格 + 规范分类 + 扫描约算**、扫描开书冻结名单与大切块、一键总结排队、近义合并、作品卡总结三态、独立 `novels/_library`、界面 / 写作多语言、按纲收束、对话人设、大纲导图、插图与分镜、应用内更新，以及 `python build.py` 默认发版流水线（见第 9 节）。

---

## 2. 它做什么

- 以**本地作品目录**为真相源：`project.json`、章节 Markdown、lore / memory / 总谱 / 分镜 JSON；对话会话另存（不混进章节）；情节 / 喜好条目存全局 `novels/_library`（列表 JSON，近义合并补充，规范分类标签可筛，可带英文短名与写法），作品内也可有同 kind lore；可由全书扫描或蒸馏写入全局库；`project.json` 可记情节总结游标（时间 / 指纹 / dirty），书名与章名可另存导入前原文 `title_src`。作品页可用书名 / 情节关键词即时过滤，并一键智能搜按大纲与情节卡做相似排序；也可把 TXT 导入成可写小说（正文不在导入时翻译）。
- 续写、润色、拆章、按纲节拍、设定召回、知识库蒸馏，都走同一套 Rust 写作引擎；按纲整章可一次写完本章纲，并自动补半句 / 收束；本章勾选的情节 / 喜好会注入生成；写作页可按段或整章把正文译成中 / 英 / 日。
- 桌面 GUI（Tauri 2 + Vue 3）负责编辑与预览；界面可切中 / 英 / 日；写作 Prompt 可按写作语言独立选择；独立「对话」页做本作 / 自由聊（可配助手人设）；独立「情节库」页以卡片网格维护套路卡、自动保存、单卡 AI 优化，英文界面显示英文标题与写法（缺则回退中文），并可批量扫描（带用量进度与确认约算）；作品首页可单卡总结或**重新总结**、**一键总结** / **一键重新总结**排队，并可用关键词过滤与智能搜排序；点作品卡立刻进写作；作品页可导入 TXT 为写作工程，标题翻译默认关闭，正文到写作页再翻。知识库与情节库导入 TXT 只选一次文件，目录自动落在 `novels`；没有章标题时按段落攒成约 3500–4500 字一块。空的作品元数据、设置和会话文件按默认或章节正文恢复，不挡打开。CLI / NDJSON RPC 给脚本编排；GUI 在线时 CLI 默认可经本机 IPC 驱动同一套预览（见 [`src-tauri/src/ipc/mod.rs`](src-tauri/src/ipc/mod.rs)、[`src/services/guiBridge.js`](src/services/guiBridge.js)）。
...
- **用量可追溯**：续写 / 润色 / 书名建议 / 导入蒸馏 / 情节扫描 / 情节卡优化 / 作品智能搜 / 正文翻译等业务 AI 调用记 token 与花费；侧栏「分析」可看余额、KPI 与趋势。
...
| 正文翻译 | 按块切段 + 已是目标语跳过 + 前端写回 | [`writing/translate.rs`](src-tauri/src/writing/translate.rs)、[`body_translate.md`](src-tauri/prompts/body_translate.md)、[`draftAccept.js`](src/services/draftAccept.js)、[`ChapterBlockEditor.vue`](src/components/ChapterBlockEditor.vue)、[`EditorView.vue`](src/views/EditorView.vue) |
...
Vue (src/)  --invoke / listen-->  Tauri commands.rs  -->  api.rs
                                                      ...
                                                      -->  writing/translate.rs -->  chapter_translate（不写盘）
...
GUI 装配：[`src-tauri/src/lib.rs`](src-tauri/src/lib.rs)（注册命令、`CancelRegistry` / `PrepareRegistry`，桌面端启动 IPC；含 `chat_session_*`、`tropes_scan`、`trope_library_ensure`、`trope_summary_status`、`work_catalog_ensure`、`novels_search`、`import_novel_txt`、`chapter_translate`；挂载 `i18n` / `prompt_i18n`）。
...
写作请求可带 `outline_run`、本章 `selected_trope_ids` 与扫描冻结名单 `known_tropes_snapshot`（[`writing/mod.rs`](src-tauri/src/writing/mod.rs)）；`trope_refine` 只优化一张情节卡、不读章；`chapter_translate` 只返回译文，由 [`draftAccept.js`](src/services/draftAccept.js) 写回。按纲队列在 [`outlineQueue.js`](src/services/outlineQueue.js) 置位。前端跨页同步：角色 / 设定用 [`bumpCastRevision()`](src/stores/appState.js)；情节库用 `tropeRevision` / [`refreshTropeIndex`](src/services/tropeIndex.js)（扫描结束后也会 bump）。大纲侧栏、总谱、设定页在 watch / `onActivated` 时重载。作品首页批量拉 [`listTropeSummaryStatus`](src/services/projectClient.js)；一键总结 / 一键重新总结走 [`scanTropesQueue`](src/services/tropeScan.js)；单卡总结 / 重新总结走 [`scanTropesFromRoot`](src/services/tropeScan.js)；智能搜走 [`novelSearch.js`](src/services/novelSearch.js)。本章勾选共用 [`tropeSelect.js`](src/services/tropeSelect.js)。
...
│   ├── services/             # Tauri / LLM / 作品 / 对话 / 插图 / 更新 / GUI 桥 / 抽角色 / 情节抽取扫描优化勾选 / 作品搜索 / 按纲队列 / 正文翻译写回
...
│   ├── src/                  # 后端模块（含 writing/translate、import/title_translate、kb、project/trope_*、work_catalog、i18n、chat、image、update）
│   ├── locales/              # 后端用户文案 zh-CN / en / ja
│   ├── prompts/              # 写作 Prompt（含 body_translate / trope_extract / trope_refine / title_translate / novel_search / scene_complete；根=zh-CN；en/、ja/）
│   ├── tauri.conf.json
...
| 作品 | [`src/views/ProjectHome.vue`](src/views/ProjectHome.vue)、[`projectClient.js`](src/services/projectClient.js)、[`novelSearch.js`](src/services/novelSearch.js) | 新建/打开/最近、仪表盘、书名建议；导入 TXT 为写作工程（标题翻译默认关，正文到写作页再翻）；关键词即时过滤与智能搜排序；点卡立刻进写作；卡片不限死高度；情节总结三态、单卡总结 / 重新总结、一键总结 / 一键重新总结排队与取消、确认约算 |
...
| 写作 | [`src/views/EditorView.vue`](src/views/EditorView.vue)、[`src/components/AiPanel.vue`](src/components/AiPanel.vue)、[`TropePickPanel.vue`](src/components/TropePickPanel.vue)、[`ChapterBlockEditor.vue`](src/components/ChapterBlockEditor.vue) | 章树（含变体 / 生成块子行）、块编辑（含插图块）、按纲队列、流式预览；切书清缓存防空载；情节/喜好卡片筛选多选；段下选语言翻译本段；工具栏翻译本章；指令按任务分槽 |
...
| i18n | [`src/i18n/index.js`](src/i18n/index.js)、[`src/i18n/locales/zh-CN.json`](src/i18n/locales/zh-CN.json)、[`en.json`](src/i18n/locales/en.json)、[`ja.json`](src/i18n/locales/ja.json) | 文案目录、`applyUiLocale`、跨语言消息匹配（含总结三态、重新总结、一键重新总结、智能搜、情节筛选、扫描约算、正文翻译文案） |
...
| 草稿落盘 | [`draftAccept.js`](src/services/draftAccept.js)、[`genJobs.js`](src/stores/genJobs.js) | `targetChapterId` / `skipAutoAccept`；错章拒写；可触发情节抽取；`translateBlock` / `translateOpenChapter` |
...
| 模块 | 路径 | 职责 |
|---|---|---|
| 共享 API | [`src-tauri/src/api.rs`](src-tauri/src/api.rs) | GUI / CLI / RPC 共用业务、`dispatch_rpc`；分镜、出图、对话会话、`chapter_update_meta`、`tropes_scan`、`trope_library_ensure`、`trope_summary_status`、`work_catalog_ensure`、`novels_search`、`import_novel_txt`、`chapter_translate`；`import_txt` 的 `root` 可空，空则在 `novels` 下自动建目录 |
| JSON 读盘 | [`src-tauri/src/error.rs`](src-tauri/src/error.rs) | `json_is_blank` / `parse_json_at`：空文件与 BOM 不当成 serde EOF；损坏文件带路径人话 |
| Tauri 命令 | [`src-tauri/src/commands.rs`](src-tauri/src/commands.rs) | `#[tauri::command]` 薄封装（含 update / image / storyboard / chat / llm_chat 流式 / `tropes_scan` / `trope_library_ensure` / `trope_summary_status` / `work_catalog_ensure` / `novels_search` / `import_novel_txt` / `chapter_translate`） |
...
| 写作引擎 | [`src-tauri/src/writing/mod.rs`](src-tauri/src/writing/mod.rs) | 任务、上下文、`run_writing`；完整度补写；`outline_run`；`{{tropes}}` 注入（读全局 `_library`，按写作语言选中/英）；`trope_refine` 只优化一张卡并补空英文；`known_tropes_snapshot` / `format_known_tropes_compact`（`中文\|english`）；Prompt 经 `prompt_i18n`；子模块 `translate` |
| 正文翻译 | [`src-tauri/src/writing/translate.rs`](src-tauri/src/writing/translate.rs) | `chapter_translate`：按块切段、已是目标语跳过、不写盘；记用量 |
...
| Prompt i18n | [`src-tauri/src/prompt_i18n.rs`](src-tauri/src/prompt_i18n.rs) | 按 `writing_locale` 选中 / 英 / 日模板（含 `scene_complete`、`trope_extract`、`trope_refine`、`novel_search`、`title_translate`、`body_translate`）；`body_translate_prompt` 跟目标语 |
...
Prompt 模板目录：[`src-tauri/prompts/`](src-tauri/prompts/)（根目录为中文；[`en/`](src-tauri/prompts/en/)、[`ja/`](src-tauri/prompts/ja/) 为英文 / 日文；如 `continue_chapter.md`、`length_fill.md`、`scene_complete.md`、`trope_extract.md`、`trope_refine.md`、`title_translate.md`、`body_translate.md`、`novel_search.md`、`outline_to_chapters.md`、`beats_to_storyboard.md`）。
...
CLI 调试优先用 `kk_novel_cli`（构建后在 `src-tauri/target/...`）。子命令说明：[`docs/lmstudio.md`](docs/lmstudio.md)。PowerShell 下任务名 `continue` 需当参数传递，避免关键字冲突。导入为写作小说示例：`kk_novel_cli import novel-txt --file "book.txt" --title "书名"`（加 `--translate` 才翻书名和章标题）。正文翻译示例：`kk_novel_cli chapter translate <root> <chapter_id> --locale en --selection "..."`（不写盘，stdout 返回译文）。情节全书扫描示例：`kk_novel_cli tropes scan D:/novels/foo --from 1 --to 0`。总结状态示例：`kk_novel_cli tropes status D:/novels/foo`。作品搜索相关 RPC：`work_catalog_ensure`、`novels_search`（`mode=local|ai`），见 [`cli.rs`](src-tauri/src/cli.rs)。
...
| 里程碑 TODO | [`docs/todo.md`](docs/todo.md) | M1–M72 及明细（均带代码路径） |
| LM Studio / CLI | [`docs/lmstudio.md`](docs/lmstudio.md) | 本地服务、按纲流程、RPC、`novels/_library`、正文翻译说明 |
...
已完成的产品里程碑见 [`docs/todo.md`](docs/todo.md)（M1–M72）。本仓库会继续收集 Issue / 用户反馈，把有效建议排进迭代。分析文档里仍开放的工程向建议：
