...
作品页「导入小说」走 **`import_novel_txt`**，建成 **`kind=novel`** 写作工程（进最近作品）。弹窗默不翻译；勾选后一次分析模型只翻书名和章标题，**正文保持原文**。打开写作页后，每段粘条可选中/英/日并点「翻译本段」；工具栏「翻译本章」会按段依次翻（长文后端自动切块，已是目标语则跳过）。
...
全局情节/喜好库：软件运行根下 `novels/_library/lore/tropes.json` 与 `kinks.json`（移动端在应用数据 `novels/_library`）。旧角色仓里的 tropes/kinks 会在首次打开情节库时迁过去。
